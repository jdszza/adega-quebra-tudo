# -*- coding: utf-8 -*-
"""
Adega PDV – Sistema Desktop (Windows)
Stack: Python 3.10+, Tkinter, MySQL (mysql-connector-python), python-escpos (opcional)

Novidades desta versão:
- Tela de Fornecedores (CRUD)
- Troca de senha e gestão de usuários (admin cria/edita; qualquer usuário troca a própria senha)
- Mais filtros (categoria/marca) na lista de produtos
- Importação CSV de produtos (upsert por SKU)
- Recibo com QR-Code PIX (estático) e identificação do atendente

ATENÇÃO
- Este é um esqueleto funcional para uso local (um PC). Ajuste conforme necessário.
- Para impressão ESC/POS, é necessário configurar a impressora em "Configurações" e instalar python-escpos.
- QR-Code PIX: usa payload estático construído a partir das configurações (Chave Pix, Nome da Loja, Cidade).
  O valor pode ser incluído no payload (se pagamento em PIX). A validação do padrão EMV/BR Code é implementada
  de forma simples (CRC16-CCITT), mas revise se precisar de total conformidade para produção.

Comandos de instalação:
    pip install mysql-connector-python
    pip install python-escpos pyusb   # (opcional – impressão)
"""
import os
import sys
import csv
...
DEFAULT_SETTINGS = {
    ...
    "network_host": "",
    "network_port": 9100,
    "pix_key": "",  # e-mail, telefone, EVP etc.
...
def init_database(db: DB):
    ...
    if count_users == 0:
        ...
        db.execute("INSERT INTO users (username, password_hash, role) VALUES (%s, %s, 'admin')", ("admin", admin_hash))
        db.commit()
...
def build_pix_payload(key: str, merchant_name: str, merchant_city: str, amount: Decimal = None, txid: str = "PDVSALE") -> str:
    """
    Constrói payload EMV/BR Code para PIX estático.
    Campos principais:
    00 - Payload Format Indicator (01)
    26 - Merchant Account Information (BR.GOV.BCB.PIX)
         00 GUI, 01 Chave, 02 Descrição (opcional)
    ...
    63 - CRC
    """
    if not key or not merchant_name or not merchant_city:
        return ""
...
class SupplierRepo:
    ...
    def upsert(self, data: dict):
        ...
        if data.get("id"):
            ...
            # se já existir fornecedor com mesmo nome, atualiza
...
class SalesRepo:
    ...
    def report_profit(self, start: datetime, end: datetime):
        cur = self.db.execute(
            """
            SELECT s.id, s.created_at, SUM(si.line_profit) AS lucro
            FROM sales s
    ...
    def report_top_products(self, start: datetime, end: datetime, limit: int = 20):
        cur = self.db.execute(
            """
            SELECT p.name, SUM(si.qty) AS qtd, SUM(si.line_total) AS total
            FROM sale_items si
    ...
    def report_low_stock(self):
        cur = self.db.execute(
            "SELECT sku, barcode, name, stock_qty, min_stock FROM products WHERE active=1 AND stock_qty <= min_stock ORDER BY stock_qty ASC"
        )
        return cur.fetchall()

    def report_expiring(self, days: int = 30):
        cur = self.db.execute(
            "SELECT sku, barcode, name, expiry, stock_qty FROM products WHERE expiry IS NOT NULL AND expiry <= %s ORDER BY expiry",
...
class LoginWindow(tk.Toplevel):
    def __init__(self, master, db: DB, on_success):
        ...
        self.title("Adega PDV - Login")
        self.geometry("320x220")
        self.resizable(False, False)
        self.grab_set()

        ttk.Label(self, text="Usuário").pack(pady=(20, 4))
        self.ent_user = ttk.Entry(self)
        self.ent_user.pack(fill="x", padx=30)
        self.ent_user.focus_set()

        ttk.Label(self, text="Senha").pack(pady=(10, 4))
        self.ent_pass = ttk.Entry(self, show="*")
        self.ent_pass.pack(fill="x", padx=30)

        self.var_show = tk.BooleanVar(value=False)
        cb = ttk.Checkbutton(self, text="Mostrar senha", variable=self.var_show, command=self._toggle_pass)
        cb.pack(pady=4)

        ttk.Button(self, text="Entrar", command=self.try_login).pack(pady=10)
    ...
    def try_login(self):
        ...
        if row and verify_password(password, row[2]):
            self.destroy()
            self.on_success({"id": row[0], "username": row[1], "role": row[3]})
        else:
            messagebox.showerror("Erro", "Usuário ou senha inválidos.")

# ===================== UI: APP PRINCIPAL =====================
class AdegaApp(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("Adega PDV")
        ...
        self.product_repo = ProductRepo(self.db)
        self.sales_repo = SalesRepo(self.db)
        self.supplier_repo = SupplierRepo(self.db)
        self.printer = ReceiptPrinter(self.db)

        # Login
        self.wait_visibility()
        LoginWindow(self, self.db, on_success=self._after_login)
    ...
    def _build_ui(self):
        ...
        top = ttk.Frame(self)
        top.pack(fill="x")
        ttk.Label(top, text=f"Usuário: {self.state['user']['username']} ({self.state['user']['role']})").pack(side="left", padx=8, pady=6)
        ttk.Button(top, text="Trocar senha", command=self.change_password_dialog).pack(side="right", padx=8)
        ttk.Button(top, text="Sair", command=self.destroy).pack(side="right", padx=8)

        nb = ttk.Notebook(self)
        nb.pack(fill="both", expand=True)

        self.tab_products = ProductTab(nb, self.product_repo)
        nb.add(self.tab_products, text="Produtos")

        self.tab_pos = PosTab(nb, self.product_repo, self.sales_repo, self.printer, self.state)
        nb.add(self.tab_pos, text="PDV")

        self.tab_reports = ReportsTab(nb, self.sales_repo)
        nb.add(self.tab_reports, text="Relatórios")

        self.tab_suppliers = SuppliersTab(nb, self.supplier_repo)
        nb.add(self.tab_suppliers, text="Fornecedores")

        self.tab_settings = SettingsTab(nb, self.db, self.printer, self.state)
        nb.add(self.tab_settings, text="Configurações")

        if self.state["user"]["role"] == "admin":
            self.tab_users = UsersTab(nb, self.user_repo, current_user_id=self.state["user"]["id"])
            nb.add(self.tab_users, text="Usuários")
    ...
    def change_password_dialog(self):
        ...
        if not row or not verify_password(current, row[2]):
        ...
        if new1 != new2:
            ...
            return
        self.user_repo.set_password(self.state["user"]["id"], new1)
...
class ProductForm(ttk.LabelFrame):
    def __init__(self, master, on_save, on_clear):
        super().__init__(master, text="Cadastro de Produto")
        ...
        self.vars['item_type'].set('Outros')
        self.vars['active'].set('1')
        ...
        for i, (lbl, key) in enumerate(grid):
            ttk.Label(self, text=lbl).grid(row=i//4, column=(i%4)*2, sticky="e", padx=4, pady=3)
            if key == 'item_type':
                cb = ttk.Combobox(self, textvariable=self.vars[key], values=['Vinho','Cerveja','Destilado','Outros'], state='readonly', width=18)
                cb.grid(row=i//4, column=(i%4)*2+1, sticky="we", padx=4, pady=3)
            else:
                ent = ttk.Entry(self, textvariable=self.vars[key])
                ent.grid(row=i//4, column=(i%4)*2+1, sticky="we", padx=4, pady=3)

        ttk.Label(self, text="Ativo (1/0)").grid(row=5, column=6, sticky="e", padx=4, pady=3)
        ttk.Entry(self, textvariable=self.vars['active']).grid(row=5, column=7, sticky="we", padx=4, pady=3)

        btns = ttk.Frame(self)
        btns.grid(row=6, column=0, columnspan=8, sticky="we", pady=6)
        ttk.Button(btns, text="Salvar/Atualizar", command=self._save).pack(side="left", padx=4)
        ttk.Button(btns, text="Limpar", command=self._clear).pack(side="left", padx=4)
    ...
    def _save(self):
        ...
        data['cost_price'] = to_decimal(data['cost_price'])
        data['margin_pct'] = to_decimal(data['margin_pct'])
        data['sale_price'] = to_decimal(data.get('sale_price') or 0)
        data['stock_qty'] = int(data['stock_qty'] or 0)
        data['min_stock'] = int(data['min_stock'] or 0)
        data['supplier_id'] = int(data['supplier_id'] or 0) or None
        data['vintage'] = int(data['vintage'] or 0) or None
        data['volume_ml'] = int(data['volume_ml'] or 0) or None
        data['abv'] = to_decimal(data['abv'] or 0)
        data['expiry'] = data['expiry'] or None
        data['active'] = int(data['active'] or 1)
        self.on_save(data)
...
class ProductTab(ttk.Frame):
    def __init__(self, master, repo: ProductRepo):
        super().__init__(master)
        self.repo = repo

        self.form = ProductForm(self, on_save=self.save_product, on_clear=self.clear_form)
        self.form.pack(fill="x", padx=8, pady=8)

        # Busca + Filtros
        searchf = ttk.Frame(self)
        searchf.pack(fill="x", padx=8)
        ...
        self.cmb_cat = ttk.Combobox(searchf, state="readonly", width=22)
        ttk.Label(searchf, text="Marca").pack(side="left", padx=(12, 2))
        self.cmb_brand = ttk.Combobox(searchf, state="readonly", width=22)
        ttk.Button(searchf, text="Filtrar", command=self.refresh_list).pack(side="left", padx=6)
        ttk.Button(searchf, text="Importar CSV", command=self.import_csv).pack(side="left", padx=6)
    ...
    def _load_filters(self):
        ...
        self.cmb_brand["values"] = brands
        self.cmb_cat.set(cats[0] if cats else "Todas")
        self.cmb_brand.set(brands[0] if brands else "Todas")

    def save_product(self, data: dict):
        try:
            if not data['sku'] or not data['name']:
                messagebox.showwarning("Atenção", "SKU e Nome são obrigatórios.")
                return
            self.repo.upsert(data)
            messagebox.showinfo("OK", "Produto salvo/atualizado.")
            self._load_filters()
            self.refresh_list()
        except Exception as e:
    ...
    def refresh_list(self):
        ...
        brand = self.cmb_brand.get().strip()
    ...
    def import_csv(self):
        ...
        if not path:
            return
        # Espera cabeçalhos: sku,barcode,name,item_type,category,brand,varietal,vintage,volume_ml,abv,country,region,supplier_id,cost_price,margin_pct,stock_qty,min_stock,lot_code,expiry,active
            ...
            for row in reader:
                try:
                    ...
                    for k in ("vintage","volume_ml","supplier_id","stock_qty","min_stock","active"):
                        if k in data:
                            data[k] = int(data[k]) if data[k] else 0
                    for k in ("cost_price","margin_pct","abv"):
                        if k in data:
                            data[k] = to_decimal(data[k])
                    if not data.get("expiry"):
                        data["expiry"] = None
                    # ajustar None para supplier_id, vintage, volume_ml
                    data["supplier_id"] = data["supplier_id"] or None
                    data["vintage"] = data["vintage"] or None
                    data["volume_ml"] = data["volume_ml"] or None
        ...
        self._load_filters()
        self.refresh_list()
        messagebox.showinfo("Importação", f"Importados: {count} | Erros: {errors}")

# ===================== UI: PDV =====================
class PosTab(ttk.Frame):
    def __init__(self, master, product_repo: ProductRepo, sales_repo: SalesRepo, printer: ReceiptPrinter, state: dict):
        super().__init__(master)
        ...
        ttk.Label(bar, text="Código de barras").pack(side="left")
        self.ent_barcode = ttk.Entry(bar)
        self.ent_barcode.pack(side="left", fill="x", expand=True, padx=6)
        self.ent_barcode.bind("<Return>", self.add_by_barcode)
        ttk.Button(bar, text="Adicionar", command=self.add_by_barcode).pack(side="left")

        # Tabela do carrinho
        cols = ("product_id","name","qty","unit_price","line_total")
        self.cart = ttk.Treeview(self, columns=cols, show="headings", height=12)
        for c in cols:
            self.cart.heading(c, text=c)
            self.cart.column(c, width=120)
        self.cart.pack(fill="both", expand=True, padx=8, pady=8)

        # Totais e pagamento
        pay = ttk.Frame(self)
        pay.pack(fill="x", padx=8, pady=6)
        ttk.Label(pay, text="Pagamento").pack(side="left")
        self.cmb_pay = ttk.Combobox(pay, values=["Dinheiro","Crédito","Débito","PIX"], state="readonly")
        self.cmb_pay.current(0)
        self.cmb_pay.pack(side="left", padx=6)

        ttk.Label(pay, text="Recebido").pack(side="left")
        self.ent_received = ttk.Entry(pay, width=12)
        self.ent_received.insert(0, "0")
        self.ent_received.pack(side="left", padx=6)

        self.lbl_subtotal = ttk.Label(pay, text="Subtotal: R$ 0,00")
        self.lbl_subtotal.pack(side="right", padx=8)
        self.lbl_total = ttk.Label(pay, text="TOTAL: R$ 0,00", font=("Segoe UI", 11, "bold"))
        self.lbl_total.pack(side="right", padx=8)

        btns = ttk.Frame(self)
        btns.pack(fill="x", padx=8, pady=6)
        ttk.Button(btns, text="Finalizar Venda (F2)", command=self.finish_sale).pack(side="left")
        ttk.Button(btns, text="Remover item", command=self.remove_selected).pack(side="left", padx=6)
        ttk.Button(btns, text="Limpar carrinho", command=self.clear_cart).pack(side="left")
        self.bind_all("<F2>", lambda e: self.finish_sale())

        self._recalc()

    def add_by_barcode(self, event=None):
        code = self.ent_barcode.get().strip()
        self.ent_barcode.delete(0, tk.END)
        if not code:
            return
        row = self.product_repo.get_by_barcode(code)
        if not row:
            messagebox.showwarning("Não encontrado", f"Código {code} não cadastrado.")
            return
        pid, name, price, stock, cost, margin = row
        if stock <= 0:
            if not messagebox.askyesno("Sem estoque", f"{name} está sem estoque. Adicionar mesmo assim?"):
                return
        ...
        for iid in self.cart.get_children():
            ...
            if int(vals[0]) == pid:
                qty = int(vals[2]) + 1
                unit_price = to_decimal(vals[3])
                line_total = (unit_price * qty).quantize(MONEY_Q)
                self.cart.item(iid, values=(pid, name, qty, f"{unit_price}", f"{line_total}"))
                self._recalc()
                return
        unit_price = to_decimal(price)
        self.cart.insert('', 'end', values=(pid, name, 1, f"{unit_price}", f"{unit_price}"))
        self._recalc()
    ...
    def _recalc(self):
        ...
        for iid in self.cart.get_children():
            vals = self.cart.item(iid, 'values')
            subtotal += to_decimal(vals[4])
        self.lbl_subtotal.config(text=f"Subtotal: {money(subtotal)}")
        self.lbl_total.config(text=f"TOTAL: {money(subtotal)}")
        return subtotal
    ...
    def finish_sale(self):
        ...
        for iid in self.cart.get_children():
            ...
            items.append({
                'product_id': int(pid), 'name': name, 'qty': int(qty),
        ...
        sale_id = self.sales_repo.create_sale(
            ...
            received=received, change_due=change
        )

        # Itens e baixa de estoque
        for it in items:
            cur = self.product_repo.db.execute("SELECT cost_price, margin_pct FROM products WHERE id=%s", (it['product_id'],))
        ...
        # Consulta venda para impressão
        cur = self.product_repo.db.execute("SELECT id, created_at, payment_method, subtotal, discount, total, received, change_due FROM sales WHERE id=%s", (sale_id,))
        sale_row = cur.fetchone()
        sale = {
            'id': sale_row[0], 'created_at': sale_row[1], 'payment_method': sale_row[2],
        ...
        for it in items:
            items_full.append({
                'name': it['name'], 'qty': it['qty'], 'unit_price': it['unit_price'], 'line_total': (it['unit_price'] * it['qty']).quantize(MONEY_Q)
            })

        # QR PIX se selecionado
        pix_payload = ""
        if payment == "PIX":
            # carrega config
            cur = self.product_repo.db.execute("SELECT store_name, pix_key, pix_merchant_city FROM settings WHERE id=1")
            st = cur.fetchone()
...
class ReportsTab(ttk.Frame):
    def __init__(self, master, sales_repo: SalesRepo):
        super().__init__(master)
        self.sales_repo = sales_repo
        ...
        ttk.Label(filt, text="De (AAAA-MM-DD)").pack(side="left")
        self.ent_start = ttk.Entry(filt, width=12)
        self.ent_start.pack(side="left", padx=4)
        ttk.Label(filt, text="Até").pack(side="left")
        self.ent_end = ttk.Entry(filt, width=12)
        self.ent_end.pack(side="left", padx=4)

        self.cmb_kind = ttk.Combobox(filt, values=["Vendas","Lucro","Mais vendidos","Baixo estoque","A vencer (30d)"], state="readonly")
        self.cmb_kind.current(0)
        self.cmb_kind.pack(side="left", padx=8)

        ttk.Button(filt, text="Gerar", command=self.generate).pack(side="left", padx=4)
        ttk.Button(filt, text="Exportar CSV", command=self.export_csv).pack(side="left")

        cols = ("c1","c2","c3","c4","c5")
        self.tree = ttk.Treeview(self, columns=cols, show="headings", height=18)
        for i, c in enumerate(cols, start=1):
            self.tree.heading(c, text=f"Col {i}")
    ...
    def _get_dates(self):
        ...
        try:
            ...
            messagebox.showerror("Erro", "Datas inválidas. Use AAAA-MM-DD.")
            return None, None
        return start, end
    ...
    def generate(self):
        ...
        if kind in ("Vendas", "Lucro", "Mais vendidos"):
            ...
            if kind == "Vendas":
                ...
                self.tree.heading("c2", text="Data/Hora")
                self.tree.heading("c3", text="Pgto")
                self.tree.heading("c4", text="Subtotal")
                self.tree.heading("c5", text="Total")
                for r in rows:
                ...
                self.tree.heading("c1", text="ID")
                self.tree.heading("c2", text="Data/Hora")
                self.tree.heading("c3", text="Lucro")
                ...
                self.tree.heading("c1", text="Produto")
                self.tree.heading("c2", text="Qtd")
                self.tree.heading("c3", text="Total")
                self.tree.heading("c4", text="-")
                self.tree.heading("c5", text="-")
                for r in rows:
            ...
            self.tree.heading("c1", text="SKU")
            self.tree.heading("c2", text="EAN")
            self.tree.heading("c3", text="Produto")
            self.tree.heading("c4", text="Estoque")
            self.tree.heading("c5", text="Mínimo")
            for r in rows:
                self.tree.insert('', 'end', values=r)
            ...
            self.tree.heading("c2", text="EAN")
            self.tree.heading("c3", text="Produto")
            self.tree.heading("c4", text="Validade")
            self.tree.heading("c5", text="Estoque")
            for r in rows:
...
class SuppliersTab(ttk.Frame):
    def __init__(self, master, repo: SupplierRepo):
        super().__init__(master)
        self.repo = repo

        frm = ttk.LabelFrame(self, text="Fornecedor")
        frm.pack(fill="x", padx=8, pady=8)

        self.var_id = tk.StringVar()
        self.var_name = tk.StringVar()
        self.var_document = tk.StringVar()
        self.var_phone = tk.StringVar()
        self.var_email = tk.StringVar()

        grid = [
            ("ID", self.var_id), ("Nome", self.var_name), ("Documento", self.var_document), ("Telefone", self.var_phone),
            ("E-mail", self.var_email)
        ]
        for i, (lbl, var) in enumerate(grid):
            ttk.Label(frm, text=lbl).grid(row=i//4, column=(i%4)*2, sticky="e", padx=4, pady=3)
            e = ttk.Entry(frm, textvariable=var)
            e.grid(row=i//4, column=(i%4)*2+1, sticky="we", padx=4, pady=3)

        btns = ttk.Frame(frm)
        btns.grid(row=2, column=0, columnspan=8, sticky="we", pady=6)
        ttk.Button(btns, text="Salvar/Atualizar", command=self.save).pack(side="left", padx=4)
        ttk.Button(btns, text="Limpar", command=self.clear).pack(side="left", padx=4)
        ...
        self.ent_search = ttk.Entry(searchf, width=30)
        self.ent_search.pack(side="left", padx=6)
        ttk.Button(searchf, text="OK", command=self.refresh).pack(side="left")
    ...
    def save(self):
        try:
            data = {
                "id": self.var_id.get().strip() or None,
            ...
            self.repo.upsert(data)
            messagebox.showinfo("OK", "Fornecedor salvo/atualizado.")
...
class SettingsTab(ttk.Frame):
    def __init__(self, master, db: DB, printer: ReceiptPrinter, state: dict):
        ...
        def row(label, key, col):
            ttk.Label(form, text=label).grid(row=row.i, column=col*2, sticky='e', padx=4, pady=3)
            ttk.Entry(form, textvariable=self.vars[key]).grid(row=row.i, column=col*2+1, sticky='we', padx=4, pady=3)
            if col==3: row.i += 1
        row.i = 0

        # Loja
        ttk.Label(form, text="Dados da Loja", font=("Segoe UI", 10, 'bold')).grid(row=row.i, column=0, sticky='w', pady=(0,6))
        row.i += 1
        row("Nome Fantasia", 'store_name', 0)
        row("Documento (CNPJ)", 'store_document', 1)
        row("Endereço", 'store_address', 2)
        row("Telefone", 'store_phone', 3)
        row("Rodapé recibo", 'receipt_footer', 0)

        # PIX
        ttk.Label(form, text="PIX", font=("Segoe UI", 10, 'bold')).grid(row=row.i, column=0, sticky='w', pady=(10,6))
        row.i += 1
        row("Chave PIX", 'pix_key', 0)
        row("Cidade (p/ QR)", 'pix_merchant_city', 1)

        # Impressora
        ttk.Label(form, text="Impressora ESC/POS", font=("Segoe UI", 10, 'bold')).grid(row=row.i, column=0, sticky='w', pady=(10,6))
        row.i += 1
        row("Habilitar (0/1)", 'print_enabled', 0)
        row("Tipo (USB/Serial/Network)", 'printer_kind', 1)
        row("USB Vendor ID (hex)", 'usb_vendor_id', 2)
        row("USB Product ID (hex)", 'usb_product_id', 3)
        row("USB IN EP (opcional)", 'usb_in_ep', 0)
        row("USB OUT EP (opcional)", 'usb_out_ep', 1)
        row("Serial COM", 'serial_device', 2)
        row("Serial baud", 'serial_baud', 3)
        row("Host IP", 'network_host', 0)
        row("Host Porta", 'network_port', 1)

        for c in range(8):
            form.columnconfigure(c, weight=1)

        ttk.Button(self, text="Salvar configurações", command=self.save).pack(pady=8)
    ...
    def save(self):
        ...
        self.db.commit()
        messagebox.showinfo("OK", "Configurações salvas.")
        self.printer._load()

# ===================== UI: USUÁRIOS (Admin) =====================
class UsersTab(ttk.Frame):
    def __init__(self, master, user_repo: UserRepo, current_user_id: int):
        super().__init__(master)
        self.repo = user_repo
        self.current_user_id = current_user_id

        frm = ttk.LabelFrame(self, text="Novo usuário")
        frm.pack(fill="x", padx=8, pady=8)

        self.var_username = tk.StringVar()
        self.var_role = tk.StringVar(value="caixa")
        self.var_pass1 = tk.StringVar()
        self.var_pass2 = tk.StringVar()

        ttk.Label(frm, text="Usuário").grid(row=0, column=0, sticky="e", padx=4, pady=3)
        ttk.Entry(frm, textvariable=self.var_username, width=24).grid(row=0, column=1, sticky="w")
        ttk.Label(frm, text="Cargo").grid(row=0, column=2, sticky="e", padx=4, pady=3)
        ttk.Combobox(frm, textvariable=self.var_role, values=["admin","gerente","caixa"], state="readonly", width=12).grid(row=0, column=3, sticky="w")
        ttk.Label(frm, text="Senha").grid(row=1, column=0, sticky="e", padx=4, pady=3)
        ttk.Entry(frm, textvariable=self.var_pass1, show="*", width=24).grid(row=1, column=1, sticky="w")
        ttk.Label(frm, text="Confirmar").grid(row=1, column=2, sticky="e", padx=4, pady=3)
        ttk.Entry(frm, textvariable=self.var_pass2, show="*", width=24).grid(row=1, column=3, sticky="w")

        ttk.Button(frm, text="Criar", command=self.create_user).grid(row=0, column=4, rowspan=2, padx=8)
        ...
        btns = ttk.Frame(self)
        btns.pack(fill="x", padx=8, pady=6)
        ttk.Button(btns, text="Resetar senha", command=self.reset_password).pack(side="left")
        ttk.Button(btns, text="Mudar cargo", command=self.change_role).pack(side="left", padx=6)
        ttk.Button(btns, text="Excluir", command=self.delete_user).pack(side="left")
    ...
    def create_user(self):
        ...
        r = self.var_role.get().strip()
        p1 = self.var_pass1.get()
        p2 = self.var_pass2.get()
        if not u or not p1:
            messagebox.showwarning("Atenção", "Usuário e senha são obrigatórios.")
            return
        if p1 != p2:
            messagebox.showerror("Erro", "Senhas não coincidem.")
            return
        try:
            self.repo.create_user(u, p1, r)
            self.var_username.set(""); self.var_pass1.set(""); self.var_pass2.set("")
            self.refresh()
    ...
    def reset_password(self):
        ...
        if not uid: return
        if uid == self.current_user_id and not messagebox.askyesno("Confirmar", "Resetar a própria senha?"):
            return
        newp = simpledialog.askstring("Resetar senha", "Nova senha:", show="*")
        if not newp: return
        self.repo.set_password(uid, newp)
        messagebox.showinfo("OK", "Senha resetada.")

    def change_role(self):
        uid = self._selected_user_id()
        if not uid: return
        role = simpledialog.askstring("Cargo", "Digite cargo (admin/gerente/caixa):")
        if not role or role not in ("admin","gerente","caixa"):
            messagebox.showerror("Erro", "Cargo inválido.")
            return
        self.repo.set_role(uid, role)
        self.refresh()
